...
function selectTab(tab,updateHash=true){for(const item of tabs){const selected=item===tab;item.setAttribute('aria-selected',String(selected));item.tabIndex=selected?0:-1;document.getElementById(item.getAttribute('aria-controls')).hidden=!selected;}window.KartMotion?.tabChange(tab,document.getElementById(tab.getAttribute('aria-controls')));if(updateHash)history.replaceState(null,'',tab.id==='tab-results'?'#community':location.pathname);}
...
for (const playLink of document.querySelectorAll('.game .play')) {
  playLink.addEventListener('click', event => {
    ...
    window.KartMotion ? window.KartMotion.openDialog(deviceDialog) : deviceDialog.showModal();
  });
}
document.querySelector('#device-cancel')?.addEventListener('click', () => window.KartMotion ? window.KartMotion.closeDialog(deviceDialog) : deviceDialog.close());
