...
await writeFile('.vercel/output/config.json',JSON.stringify({version:3,routes:[{src:'/(.*)',headers:{'X-Robots-Tag':'noindex, nofollow','X-Content-Type-Options':'nosniff'},continue:true},{src:'/api/ratings/?$',dest:'/api/ratings'},{src:'/api/game-health/?$',dest:'/api/game-health'},{handle:'filesystem'}]},null,2));
...
const healthFn=".vercel/output/functions/api/game-health.func";
await mkdir(healthFn,{recursive:true});
await cp("server/game-health.mjs",`${healthFn}/index.mjs`);
await writeFile(`${healthFn}/.vc-config.json`,JSON.stringify({runtime:"nodejs24.x",handler:"index.mjs",launcherType:"Nodejs",maxDuration:15}));
