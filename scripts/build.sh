...
: "${GAME_E_SERVER_URL:?Set the deployed HyperKart backend URL}"
(cd games/e && npm ci --ignore-scripts --no-audit --no-fund && BASE_PATH=/games/e/ VITE_GAME_SERVER_URL="$GAME_E_SERVER_URL" npm run build)
